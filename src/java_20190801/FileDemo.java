...
public class FileDemo {
	...
	public String getComma(long name) {
		return String.format("%,d", name);        //콤마 찍는거
	}
	...
	public void changeFile(String path) {
		//path => c:\dev\test\jdk.exe => c:\dev\test\12312312312313.exe
		File f1 = new File(path);
		//f1.getPath() => 전체경로 + 파일이름
		String fileName = f1.getName();//전체 경로 중에 파일이름만 반환
	...
	public static void main(String[] args) {
		...
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a hh:mm E요일");
		String date = sdf.format(c.getTime());
		System.out.println(date);
		
		File f5 = new File("c:\\dev\\test");
		String[] lists = f5.list();
		File f6 = null; //메모리를 안남게 하려면
		for (String list : lists) {
						//f5.getPath() 무조건  String만 오는 줄 아는 바보들만 사용 => f5로 충분
			f6 = new File(f5,list);
